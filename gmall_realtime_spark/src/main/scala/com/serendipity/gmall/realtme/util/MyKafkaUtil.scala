...
object MyKafkaUtil {
  ...
  private val kafkaConsumerParams: mutable.Map[String, String] = mutable.Map(
    //kafka连接工具
    ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG -> PropertiesUtils.apply("kafka.broker.list"),
    //kafka中key的序列化方式
  ...
  private val kafkaProducerParams: mutable.Map[String, String] = mutable.Map(
    ProducerConfig.BOOTSTRAP_SERVERS_CONFIG -> PropertiesUtils.apply("kafka.broker.list"),
    ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG -> "org.apache.kafka.common.serialization.StringDeserializer",
    ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG -> "org.apache.kafka.common.serialization.StringDeserializer",
  ...
  def createKafkaProducer(): KafkaProducer[String, String] ={
    ...
    kafkaProducerProperties.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG,PropertiesUtils.apply("kafka.broker.list"));
    kafkaProducerProperties.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG,"org.apache.kafka.common.serialization.StringDeserializer")
    kafkaProducerProperties.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG,"org.apache.kafka.common.serialization.StringDeserializer")
    //开启幂等性：所谓的幂等性就是指Producer不论向Server发送多少次重复数据，Server端都只会持久化一条
